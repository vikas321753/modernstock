module.exports = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // Log/ignore search fallback
        }

        // Step 2: Fetch details
        const yfUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(resolvedSymbol)}?interval=1d&range=7d`;
        const yfRes = await fetch(yfUrl, { headers: { 'User-Agent': userAgent } });
        
        if (!yfRes.ok) {
            res.status(yfRes.status).json({ error: `Yahoo Finance responded with status ${yfRes.status}` });
            return;
        }

        const json = await yfRes.json();
        if (!json.chart || !json.chart.result || json.chart.result.length === 0) {
            res.status(404).json({ error: 'Stock details not found' });
            return;
        }
